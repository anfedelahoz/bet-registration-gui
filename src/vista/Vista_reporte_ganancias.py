...
class Vista_reporte_ganancias(QWidget):
    ...
    def mostrar_ganancias(self, lista_ganancias, ganancias_casa):
        ...
        numero_fila = 1
        for _apostador_valor in lista_ganancias:
            _nombre_apostador=str(_apostador_valor.apostador)
            print(_nombre_apostador)
            etiqueta_viajero = QLabel(_nombre_apostador)
            etiqueta_viajero.setWordWrap(True)
            self.distribuidor_tabla_reporte.addWidget(etiqueta_viajero, numero_fila, 0, Qt.AlignLeft)

            etiqueta_total = QLabel("${:,.2f}".format(_apostador_valor.valor))
