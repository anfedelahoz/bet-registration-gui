...
class Dialogo_crear_apuesta(QDialog):
    ...
    def __init__(self, apostadores, competidores, apuesta=None):
        ...
        for apostador in self.apostadores:
            #print(apostador.__dict__)
            _nombreApostador=apostador.nombre
            _idApostador=apostador.id_apostador
            self.combobox_apostadores.addItem(_nombreApostador,userData=_idApostador)
        ...
        for competidor in self.competidores:
            _nombreCompetidor=competidor.nombre
            _idCompetidor=competidor.id_competidor
            self.combobox_competidores.addItem(_nombreCompetidor, userData=_idCompetidor)             
        ...
        if apuesta != None:
            print("ingresa a if apuesta != None:")
            self.texto_valor.setText(str(apuesta.valor))
            indice_apostador = self.combobox_apostadores.findData(apuesta.apostador)
            self.combobox_apostadores.setCurrentIndex(indice_apostador)
            
            #self.combobox_apostadores.lineEdit().setReadOnly(True)
            #self.combobox_apostadores.line_edit.setReadOnly(True)
