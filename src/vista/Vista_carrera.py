...
class Vista_carrera(QWidget):
    ...
    def aniadir_competidor(self):
        ...
        if dialogo.resultado == 1:
            _nombre=dialogo.texto_nombre.text()
            _probabilidad=dialogo.texto_probabilidad.text()
            esnumerico=True
            try:
                _probabilidad=float(_probabilidad)
                # if(_probabilidad>=1):
                #     self.interfaz.show_warning_messagebox("el valor de la probabilidad deber ser numerico y menor a 1....")                  

            except ValueError:
                esnumerico=False
            if (esnumerico==True and all([isinstance(_probabilidad, (int,float))]) and len(_nombre)>0 and  _probabilidad<1) : 
                ...
                self.interfaz.show_warning_messagebox("el nombre es obligatorio ,el valor de la probabilidad deber ser numerico y menor a 1....")
    ...
    def guardar_cambios(self):
        ...
        sumatoria_probabilidad=0.0   
        self.interfaz.inicializar_competidores() 
        if(self.competidores != None and len(self.competidores)>=2):
            for i, competidor in enumerate(self.competidores):
                sumatoria_probabilidad= sumatoria_probabilidad +  float(competidor['Probabilidad'])
                if competidor.get('Estado') == 'Nueva':
                    self.interfaz.aniadir_competidor(self.texto_nombre.text(), competidor['Nombre'], competidor['Probabilidad'])
                else:
                    self.interfaz.editar_competidor(i, competidor['Nombre'], competidor['Probabilidad'])
            
            if(sumatoria_probabilidad>1):
                self.interfaz.show_warning_messagebox("no se puede completar la acción, la sumatoria de las probabilidades de todos los competidores no puede ser superior a 1....")
            else:
                self.interfaz.guardar_carrera(self.texto_nombre.text())
                self.hide()
                self.interfaz.mostrar_vista_lista_carreras()
        else:
            self.interfaz.show_warning_messagebox("no se puede completar la acción, se requieren minimos dos competidores....")
